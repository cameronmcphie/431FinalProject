...
class Address
{
   ...
   function email()
   {
     ...
     {
       $this->email = htmlspecialchars(trim(func_get_arg(0)));
     }
   ...
   function __construct($name="", $street="", $city="", $state="", $country="", $zip="", $email="")
   {
   ...
   function toTSV()
   {
       return implode("\t", [$this->name(), $this->street(), $this->city(), $this->state(), $this->country(), $this->zip(), $this->email()]);
   }
   ...
   function fromTSV(string $tsvString)
   {
     ...
     list($name, $street, $city, $state, $country, $zip, $email) = explode("\t", $tsvString);
     $this->name($name);
     $this->street($street);
     $this->city($city);
     $this->state($state);
